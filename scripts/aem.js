 ...
 * @param {string} globalCheck optional global variable name to wait for
 */
async function loadScript(src, attrs, globalCheck = null) {
  return new Promise((resolve, reject) => {
    if (!document.querySelector(`head > script[src="${src}"]`)) {
      ...
      script.onload = () => {
        if (globalCheck) {
          // Wait for the specific global to be available with timeout protection
          let retries = 0;
          const maxRetries = 100; // 1 second max wait (100 * 10ms)

          const checkGlobal = () => {
            if (window[globalCheck] !== undefined) {
              resolve();
            } else if (retries >= maxRetries) {
              reject(new Error(`Global variable '${globalCheck}' not available after ${maxRetries * 10}ms`));
            } else {
              retries += 1;
...
function decorateSections(main) {
  main.querySelectorAll(':scope > div:not([data-section-status])').forEach((section) => {
    ...
    if (section.dataset.background) {
      const backgroundColor = section.dataset.background;
      if (backgroundColor) {
    ...
    if (section.dataset.backgroundimage) {
      const imageUrl = section.dataset.backgroundimage;
      section.style.setProperty('--section-bg-image', `url("${imageUrl}")`);
    }

    // Handle background overlay if present
    if (section.dataset.backgroundoverlay) {
      const overlayColor = section.dataset.backgroundoverlay;
      if (overlayColor) {
        section.style.setProperty('--section-bg-overlay', 'linear-gradient(0deg, rgba(0, 0, 0, 0.80) 0%, rgba(0, 0, 0, 0.80) 100%)');
      }
    }
  });
}
