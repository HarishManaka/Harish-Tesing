...
import { trackHistory, initializeCartScrollListener } from './commerce.js';
 ...
 */
export function rewriteDamLinks() {
...
/**
 * Handles external links by opening them in new tabs
 * Excludes commerce team's specified page wrappers
 * @param {Element} container The container element to search for links
 */
export function handleExternalLinks(container) {
  // Check if we're inside an excluded commerce wrapper
  const excludedWrappers = [
    'commerce-checkout-wrapper',
    'commerce-cart-wrapper',
    'order-confirmation',
  ];

  const isExcluded = excludedWrappers.some((className) => container.closest(`.${className}`) || container.classList.contains(className));

  if (isExcluded) {
    return; // Don't process links in excluded areas
  }

  const links = container.querySelectorAll('a[href]');

  links.forEach((link) => {
    // Check if the link itself is inside an excluded wrapper
    const linkIsExcluded = excludedWrappers.some((className) => link.closest(`.${className}`));

    if (linkIsExcluded) {
      return; // Skip links inside excluded wrappers
    }
...
/**
 * Adds custom behavior to all sections
 * @param {Element} main The main element
 */
function decorateAllSections(main) {
  const sections = main.querySelectorAll('.section');
  sections.forEach((section) => {
    // Handle section ID from data-id attribute for anchor links
    if (section.dataset.id) {
      section.id = section.dataset.id;
    }

    // Add your custom section behavior here
    ...
    // Add marquee if data-marquee attribute exists
    const marqueeText = section.dataset.marquee;
    if (marqueeText) {
      const footerCta = document.createElement('div');
      footerCta.className = 'footer-cta';

      // Create two ul elements for seamless scrolling
      for (let i = 0; i < 1; i += 1) {
        const ul = document.createElement('ul');
        ul.className = 'footer-cta-content';

        // Add aria-hidden to the second ul for accessibility
        if (i === 1) {
          ul.setAttribute('aria-hidden', 'true');
        }

        // Create two li elements with the marquee text
        for (let j = 0; j < 1; j += 1) {
        ...
        footerCta.appendChild(ul);
      }

      section.appendChild(footerCta);
    }

    // Add any other section-specific behavior
    if (section.dataset.background) {
      // Custom behavior for sections with background
    }

    if (section.classList.contains('highlight')) {
      // Custom behavior for highlight sections
    }
  });
}
...
async function loadLazy(doc) {
  ...
  // Handle external links in main content (excluding commerce wrappers)
  if (main) {
    handleExternalLinks(main);
  }
  ...
  await Promise.all([
    loadHeader(doc.querySelector('header')).then(() => {
      // Handle external links in header after it's loaded
    ...
    loadFooter(doc.querySelector('footer')).then(() => {
      // Handle external links in footer after it's loaded
      const footer = doc.querySelector('footer');
      if (footer) {
        handleExternalLinks(footer);
      }
    }),
  ...
  // Initialize cart scroll listener for auto-scroll to cart icon after add-to-cart
  initializeCartScrollListener();
...
function loadDelayed() {
  // GTM is now loaded in strict order from delayed.js (Ensighten → Optimizely → GTM)
  window.setTimeout(() => {
    console.info('📦 Delayed phase: Importing delayed.js for third-party scripts...');
    import('./delayed.js');
  }, 3000);
...
/**
 * Loads the Swiper library from CDN with proper error handling and verification.
 * @returns {Promise<void>} Promise that resolves when Swiper is loaded and ready
 */
export async function loadSwiper() {
  ...
  // Load Swiper CSS if not already loaded
    ...
    try {
      // Load script and wait for Swiper global to be available
      await loadScript('https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.js', null, 'Swiper');
    } catch (cdnError) {
      console.warn('Failed to load Swiper from CDN:', cdnError);
      throw new Error('Could not load Swiper library');
    }
  }
}
