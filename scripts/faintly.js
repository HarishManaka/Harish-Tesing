...
async function resolveTemplate(context) {
  ...
  context.template.path = context.template.path
    || `${context.codeBasePath}/blocks/${context.blockName}/${context.blockName}.html`;

  const templateId = `faintly-template-${context.template.path}#${
...
async function processTest(el, context) {
  const testAttrName = el
    .getAttributeNames()
    .find(
      (attrName) => attrName.startsWith('data-fly-test')
        || attrName.startsWith('data-fly-not'),
    );
  ...
  const testResult = testAttrName.startsWith('data-fly-not')
    ? !testData
    : !!testData;
