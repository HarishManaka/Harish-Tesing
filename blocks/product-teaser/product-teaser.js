...
import {
  performCatalogServiceQuery, renderPrice, setupAddToCartDataLayer, trackAddToCartGA4,
...
function renderProduct(product, config, block) {
  ...
  if (addToCartButton) {
    addToCartButton.addEventListener('click', async () => {
      ...
      // Setup enhanced data layer with NASM-specific attributes
      setupAddToCartDataLayer(product, 1, []);

      // Track add_to_cart event using Commerce team's GA4 approach
      await trackAddToCartGA4(product, 1);
