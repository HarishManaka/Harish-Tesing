...
import { performCatalogServiceQuery, fetchPlaceholders } from '../../scripts/commerce.js';
...
function renderItem(unitId, product) {
  ...
  const ctaText = product.__typename === 'SimpleProductView' ? 'Add' : 'Select Options';
    ...
    <a href="${rootLink(`/products/${product.urlKey}/${product.sku}`)}">
      <div class="product-grid-image-wrapper">
        ...
        <button class="product-details-button">
          ...
          <span class="details-text">${labels?.product.recommendations.detail}</span>
        </button>
      </div>
      <span class="product-grid-name">${product.name}</span>
      ${product?.price ? `<span class="product-grid-price">$${product.price?.final?.amount?.value}</span>` : ''}
    </a>
    <span class="product-grid-cta">${ctaText}</span>
  </div>
`);
...
function renderItems(block, results) {
  ...
  // Title
  block.querySelector('h4').textContent = labels?.cart.recommendations.title;
