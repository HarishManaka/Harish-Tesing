...
class ProductCard extends Component {
  ...
  renderImage(loading = 'lazy') {
    ...
    return html`<picture>
      ...
      <img class="product-image-photo" src="${url}" max-width="330" max-height="396" alt=${(() => {
  const marketingProductName = product.attributes?.find((attr) => attr.name === 'marketing_product_name')?.value;
  return marketingProductName || product.name;
})()} loading=${loading} />
    </picture>`;
  }
  ...
  getProductUrl(product) {
    // Get pdp_type from product attributes
    const pdpTypeRaw = product.attributes?.find((attr) => attr.name === 'pdp_type')?.value;

    // If no pdp_type found, use existing route
    if (!pdpTypeRaw) {
      return rootLink(`/products/${product.urlKey}/${product.sku}`);
    }
    ...
    const pdpType = pdpTypeMap[pdpTypeRaw] || pdpTypeRaw;

    // Route based on pdp_type
    switch (pdpType) {
      case 'Subscription':
        return rootLink(`/products/subscription/${product.urlKey}/${product.sku}`);
      case 'Bundle':
        return rootLink(`/products/bundle/${product.urlKey}/${product.sku}`);
      case 'Matrix':
        return rootLink(`/products/matrix/${product.urlKey}/${product.sku}`);
      case 'Single_Simple':
        return rootLink(`/products/single-simple/${product.urlKey}/${product.sku}`);
      case 'Single_Complex':
        return rootLink(`/products/single-complex/${product.urlKey}/${product.sku}`);
      default:
        // Default route for any unknown pdp_type values
        return rootLink(`/products/${product.urlKey}/${product.sku}`);
    }
  }
  ...
  render({
    ...
    return html`
      <li index=${index} ref=${secondLastProduct}>
        <div class="picture">
          <a onClick=${() => this.onProductClick(product)} href="${this.getProductUrl(product)}">
            ${this.renderImage(index < numberOfEagerImages ? 'eager' : 'lazy')}
          </a>
        </div>
        <div class="name">
          <a onClick=${() => this.onProductClick(product)} href="${this.getProductUrl(product)}" dangerouslySetInnerHTML=${{
  __html: (() => {
  // Get marketing_product_name from attributes array, fallback to regular name
    const marketingProductName = product.attributes?.find((attr) => attr.name === 'marketing_product_name')?.value;
    return marketingProductName || product.name;
  })(),
}} />
